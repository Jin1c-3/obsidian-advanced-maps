...
description: '在 Obsidian Bases 地图上给每篇笔记单独设置 Lucide 图标和颜色，用 Base 公式批量驱动，也能看懂图标名写错后出现的空白实心圆。'
...
本页整理这两个选项各自接受什么值、哪些图标名画得出来、哪些只会留下空白实心圆，以及
...
> [!WARNING]
> 图标名必须和 Lucide 自己的拼写**完全一致**。不一致时，既不会报错，也不会退回小
> 圆点——你得到的是一个**没有图标的实心圆**，控制台里也不会有任何输出。

![三列真实的地图图钉：五个画出了图标，四个只剩没有图标的实心圆，两个退回了小圆点](../../images/marker-icon-names.png)
...
| `Star`                         | **没有图标的实心圆**——名字区分大小写                 |
| `map_pin`                      | **没有图标的实心圆**——Lucide 用 `-` 连词，从不用 `_` |
| `🌲`                           | **没有图标的实心圆**——emoji 不是图标名               |
| 任何 Lucide 没有的名字         | **没有图标的实心圆**                                 |
| 属性缺失或为空                 | 默认的小圆点                                         |
| 字面量文本 `null`、`undefined` | 默认的小圆点                                         |

所以没有图标的实心圆说明名字拼错了，小圆点则说明值没填。看一眼图钉，就知道该查哪边。
...
> [!WARNING]
> CSS 解析不了的值同样不会报错。`not-a-color`、`#zzz`，以及指向一个不存在的变量
> 的 `var()`，最后都会解析成**主题的正文文字颜色**——亮色主题下接近黑，暗色主题
> 下接近白。图钉突然变成这种纯色，通常说明颜色值无效；没填属性则会使用默认蓝色。
