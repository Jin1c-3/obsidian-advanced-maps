...
With them off, no pack SHALL be resolved, offered or drawn anywhere — not on a
base map view, not on an inline map, and not among a map's own options — and the
host's own background control SHALL be left holding the host's own list of
backgrounds, so a background the reader adds where the host configures its own
reaches an open map's control the next time that control is opened.

This is the feature whose on state costs a reader who does not use it, so the
setting SHALL default to on only for a reader who already has a pack configured,
and to off for everyone else.

Switching it back on SHALL make every configured pack drawable and pickable
again — through the map's own options at once, and where the host offers its
backgrounds only once that map is opened again, because the host's control is
handed what it offers only when it is built.
