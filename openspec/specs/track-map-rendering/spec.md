...
Measurements contributed to a popup are the track-statistics capability's
figures, measured as it states — from the track's own unshifted WGS-84 data
rather than from the geometry drawn in the map's tile datum, so a popup and an
inline embed of one file report the same numbers.
