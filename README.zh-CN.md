...
Advanced Maps 可以直接读取整个照片目录的 GPS，绘制 GPX/GeoJSON/KML/TCX 轨迹与面，并根据
...
GeoJSON 和 KML 里装的可以是面，而不是轨迹。面用同一个笔记颜色填充，边界用同样的线宽
描出，洞仍然是洞。面不带方向箭头，也没有起点终点标记，不计入内联地图下方的统计；点击
时它排在最后——落在面里的图钉、途经点或照片，点击仍然归它们自己。

![从 GeoJSON 和 KML 读出的面：按所属笔记颜色填充，其中两个带未填充的洞，一条带绿色起点、红色圆环终点和方向箭头的轨迹穿过其中一个](docs/area-fill.png)
