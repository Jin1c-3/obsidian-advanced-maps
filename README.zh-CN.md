...
![原生 Maps 视图的 base 就是一个装了 1811 张定位照片的目录：缩略图按 EXIF 位置散布全国，缩放放不下缩略图的照片仍保留一个圆点](docs/photo-album.png)
...
截图只使用第三方底图和搜索服务来演示插件，服务商署名保留在画面中。演示笔记是合成的；
照片和穿过它们的路线是作者自己的，不含私人题材，画面中没有人脸、也没有可辨认的人物。
上面那张全国相册同样是演示数据：把这些照片复制到真实地标坐标上，好让一张截图说明大型
相册的效果，而不用公开任何人去过哪里。
